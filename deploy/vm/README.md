...
| `npm run db:backfill-kill-storage` | Optional: copy participant guild/alliance cols, null participant JSONB, slim event payloads. `--dry-run` first. Resume with `--phase=null-payload --batch=20000` or `--phase=slim-events` |
